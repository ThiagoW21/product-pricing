...
async function updateProductPack(transaction, product, pack) {
  ...
  const newSalesPrice = productPack.sales_price - oldProductTotal + newProductTotal;
...
async function updateProductsPacks(transaction, product) {
  ...
  await Promise.all(packs.map((pack) => updateProductPack(transaction, product, pack)));
}
...
async function updateProduct(productToUpdate) {
  ...
  if (!productInDb) {
    throw newError('One or more products cannot be found to update', 'BadRequest', 400);
  }
  
  const product = { ...productToUpdate, oldPrice: productInDb.sales_price };

  const { is_valid: isValid } = validateNewPrice(productInDb, product.new_price);

  if (!isValid) {
    throw newError('One or more products have invalid prices', 'BadRequest', 400);
  }
...
async function updateProducts(productsToUpdate) {
  const productsUpdated = await Promise.all(productsToUpdate.map(updateProduct));
